...
impl Env {
    pub fn new(
        ...
        if cgroups_args.is_empty() {
            let builder = CgroupConfigurationBuilder::new(proc_mounts)?;
        ...
        if let Some(cgroups_args) = arguments.multiple_values("cgroup") {
            let mut builder = CgroupConfigurationBuilder::new(proc_mounts)?;
    ...
    fn validate_exec_file(exec_file: &str) -> Result<(PathBuf, String), JailerError> {
        ...
        if !exec_file_name.contains("cloud-hypervisor") {
    ...
    fn copy_exec_to_chroot(&mut self) -> Result<OsString, JailerError> {
        ...
        //    Cloud-Hypervisor binary (like the executable .text section), this latter part is not
        //    desirable in our threat model. Copying prevents 2 Cloud-Hypervisor processes from
        //    sharing memory.
...
mod tests {
    ...
    fn get_pseudo_exec_file_path() -> String {
        format!(
            "/tmp/{}/pseudo_cloud_hypervisor_exec_file",
    ...
    fn test_validate_exec_file() {
        ...
        std::fs::create_dir_all("/tmp/cloud-hypervisor_test_dir").unwrap();
        assert_eq!(
            format!(
                "{}",
                Env::validate_exec_file("/tmp/cloud-hypervisor_test_dir").unwrap_err()
            ),
            "/tmp/cloud-hypervisor_test_dir is not a file"
        );

        // Error case 3: Filename without "cloud-hypervisor"
        File::create("/tmp/cloud-hypervisor_test_dir/foobarbaz").unwrap();
        assert_eq!(
            format!(
                "{}",
                Env::validate_exec_file("/tmp/cloud-hypervisor_test_dir/foobarbaz").unwrap_err()
            ),
            "Invalid filename. The filename of `--exec-file` option must contain \"cloud-hypervisor\": \
             foobarbaz"
        );
        std::fs::remove_file("/tmp/cloud-hypervisor_test_dir/foobarbaz").unwrap();
        std::fs::remove_dir_all("/tmp/cloud-hypervisor_test_dir").unwrap();
    }
